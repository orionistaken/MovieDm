...
dosya_yolu = "C:\\Users\\bkork\\OneDrive\\Documents\\Julia\\OdevDm\\url.txt"
urls = []

open(dosya_yolu, "r") do dosya
    for satir in eachline(dosya)
        satir = strip(satir)
        if startswith(satir, "/title/")
            push!(urls, "https://www.imdb.com" * satir)
        end
    end
end
